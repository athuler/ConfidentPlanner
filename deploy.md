...
re-run the `gsutil rsync` / `gsutil cp` lines above, then push any commit to `main` (or `gcloud builds submit --config cloudbuild.yaml`)
to bake the new files into a fresh image.
